...
document.getElementById("block-button").addEventListener("click", () => {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    const origin = `*://*.${basename}/*`;

    // Chrome closes the popup as soon as the native permission dialog opens
    // (it steals focus). Record the intent in storage first so the background
    // service worker can finish the job via `chrome.permissions.onAdded`.
    chrome.storage.local.set(
      { pendingBlock: { basename, tabId: tab.id, at: Date.now() } },
      () => {
        chrome.permissions.contains({ origins: [origin] }, (has) => {
          if (has) {
            // Already granted - we can run the full flow inline and keep the
            // "blocked" confirmation alert.
            chrome.runtime.sendMessage(
              { action: "detectAndBlock", basename, tabId: tab.id },
              () => {
                if (chrome.runtime.lastError)
                  console.error(chrome.runtime.lastError.message);
                else alert("Your visits are now blocked on this site.");
              }
            );
            return;
          }

          // First-time grant: request the origin. The popup will be destroyed
          // once the dialog opens; background takes over on permissions.onAdded.
          chrome.permissions.request({ origins: [origin] }, (granted) => {
            ...
            // Rarely reached (popup usually closed before we get here). If it
            // is, fire the same flow so the user gets confirmation too.
            chrome.runtime.sendMessage(
              { action: "detectAndBlock", basename, tabId: tab.id },
              () => {
                if (chrome.runtime.lastError)
...
// Copied from https://publicsuffix.org/list/public_suffix_list.dat
// Last copied on 21-04-2026
// How to convert that format?
//  1) Remove these lines with regex: //(.*)\n
//  2) Remove empty lines with regex: ^$\n
//  3) Replace without regex "*." with ""
const tldsWithDots = [
  "0.bg",

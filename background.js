...
const blockRequests = function (details) {
  ...
  if (found) {
    ...
    if (tabId) {
      tabs = tabs || {};
      const tabText = "" + tabId;
      if (tabs[tabText] && !isSame) tabs[tabText]++;
      else if (!tabs[tabText]) tabs[tabText] = 1;

      chrome.browserAction.setBadgeText({ text: "" + tabs[tabId], tabId });
      if (IS_FIREFOX) {
...
chrome.runtime.onInstalled.addListener(function () {
  ...
  chrome.browserAction.setBadgeBackgroundColor({ color: "#FF4F64" });
});
...
chrome.storage.onChanged.addListener(
  ({ blacklist: storageBlacklist, scripts: storageScripts }, areaName) => {
    ...
    if (storageScripts) {
      scripts = [...storageScripts.newValue];
...
const checkForScripts = (basename) =>
    ...
    chrome.tabs.executeScript(
      ...
      function ([scripts] = []) {
        ...
        const cleanScripts = (scripts || []).filter((item) => {
          ...
          return /^(\/v[0-9]+)?\/(app|latest|e|events|light)\.js$/i.test(
            pathname
          );
        });
...
chrome.browserAction.onClicked.addListener(function (tab) {
  ...
  if (
    ...
    return requestPermissionForUrl(basename, waitingPermissions[basename]);
